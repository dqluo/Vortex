...
public class WorkerStatusServlet extends HttpServlet {
       ...
       throws java.io.IOException
  {
    ...
    int keysWritten = Integer.valueOf(request.getParameter(AllStrings.crawlcountString));
    int currentSeed = Integer.valueOf(request.getParameter(AllStrings.seedToStartWithString));
    int creationcount = Integer.valueOf(request.getParameter(AllStrings.creationCountString));
    int lineCountOfBFR = Integer.valueOf(request.getParameter(AllStrings.lineReadTillInBufferedReaderString));
    Master.updateWorkerStatus(workerPort,workerhost, status,keysWritten,currentSeed,creationcount,lineCountOfBFR );//, keysRead, keysWritten);
    ...
    if(status.equals("waiting"))
    {
    	...
    	{
    		Master.callRunmap(workerhost,workerPort,currentSeed,creationcount);
    	}
    }
